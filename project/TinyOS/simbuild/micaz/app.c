...
typedef TMilli MqttC$TimerSub$precision_tag;
typedef TMilli MqttC$TimerNextPub$precision_tag;
typedef TMilli MqttC$TimerConn$precision_tag;
typedef TMilli MqttC$TimerNextSub$precision_tag;
...
error_t error);
# 83 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
static void MqttC$TimerSub$fired(void );
#line 83
static void MqttC$TimerNextPub$fired(void );
# 78 "/opt/tinyos-main-2.1.2/tos/interfaces/Receive.nc"
static 
#line 74
message_t * 
...
uint8_t len);
# 83 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
static void MqttC$TimerConn$fired(void );
#line 83
static void MqttC$TimerNextSub$fired(void );
# 80 "/opt/tinyos-main-2.1.2/tos/interfaces/AMSend.nc"
...
# 92 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
static bool MqttC$Timer0$isRunning(void );
#line 73
...
uint8_t len);
# 92 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
static bool MqttC$TimerSub$isRunning(void );
#line 73
static void MqttC$TimerSub$startOneShot(uint32_t dt);




static void MqttC$TimerSub$stop(void );
#line 73
static void MqttC$TimerNextPub$startOneShot(uint32_t dt);
# 52 "/opt/tinyos-main-2.1.2/tos/interfaces/Random.nc"
static uint16_t MqttC$Random$rand16(void );
# 92 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
static bool MqttC$TimerConn$isRunning(void );
#line 73
static void MqttC$TimerConn$startOneShot(uint32_t dt);




static void MqttC$TimerConn$stop(void );
#line 73
static void MqttC$TimerNextSub$startOneShot(uint32_t dt);
# 31 "MqttC.nc"
message_t MqttC$packet[1000];
message_t MqttC$queued_packet[1000];
uint16_t MqttC$queue_addr[1000];
uint16_t MqttC$time_delays[1000][9];


bool MqttC$connection_status[1000];

bool MqttC$current_sub_status[1000];

uint8_t MqttC$subscribed_topic_index[1000];

uint16_t MqttC$connection_status_devices[1000][10];

uint8_t MqttC$cd_index[1000];


const uint8_t MqttC$TEMPRATURE[1000];
const uint8_t MqttC$HUMIDITY[1000];
const uint8_t MqttC$LUMINOSITY[1000];
const char *MqttC$topic_name[1000][3];

uint8_t MqttC$publish_topic[1000];
int MqttC$sent_index[1000][3];

uint16_t MqttC$message_queue[1000][3];

uint8_t MqttC$message_list[1000][3];

bool MqttC$locked[1000];



uint8_t MqttC$subscribed_topic[1000][3];

bool MqttC$subscription_matrix[1000][3][8];
...
static void MqttC$send_connect_msg(void );
static inline void MqttC$send_connect_msg_ack(uint16_t dest);
static void MqttC$sub_msg_send(uint8_t topic);
static inline void MqttC$send_connect_msg_ack(uint16_t dest);
static inline void MqttC$send_pub_msg(void );
static void MqttC$publish(void );
static inline bool MqttC$actual_send(uint16_t address, message_t *packet);


static bool MqttC$generate_send(uint16_t address, message_t *packet);
...
static inline void MqttC$TimerConn$fired(void );
...
static inline void MqttC$TimerSub$fired(void );
...
static inline void MqttC$TimerNextSub$fired(void );
#line 213
static inline void MqttC$TimerNextPub$fired(void );
...
static void MqttC$sub_msg_send(uint8_t topic);
#line 276
static inline void MqttC$sub_msg_ack(uint16_t dest, uint8_t topic);
#line 292
static inline void MqttC$send_pub_msg(void );
#line 310
static void MqttC$publish(void );
...
#line 488
static void MqttC$AMSend$sendDone(message_t *bufPtr, error_t error);
# 110 "/opt/tinyos-main-2.1.2/tos/interfaces/AMSend.nc"
...
static inline error_t /*MqttAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMSend$send(am_addr_t dest, 
message_t *msg, 
uint8_t len);
...
inline static void MqttC$TimerSub$stop(void ){
#line 78
...
# 92 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static bool MqttC$TimerSub$isRunning(void ){
#line 92
...
#line 276
{
  mqtt_packet_t *suback_msg = (mqtt_packet_t *)MqttC$Packet$getPayload(&MqttC$packet[sim_node()], sizeof(mqtt_packet_t ));
...
#line 278
  ...
  sim_log_debug(155U, "subscription", "SubAck message for topic %s(%u) sent to the node %u.\n", MqttC$topic_name[sim_node()][topic - 1], topic, dest);
  MqttC$generate_send(dest, &MqttC$packet[sim_node()]);
}

# 78 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static void MqttC$TimerConn$stop(void ){
#line 78
...
#line 78
#line 92
inline static bool MqttC$TimerConn$isRunning(void ){
#line 92
  unsigned char __nesc_result;
#line 92
...
#line 238
{
  mqtt_packet_t *conn_ack = (mqtt_packet_t *)MqttC$Packet$getPayload(&MqttC$packet[sim_node()], sizeof(mqtt_packet_t ));
...
#line 240
  ...
  sim_log_debug(153U, "connection", "ConnAck message sent to the node %u.\n", dest);
  MqttC$generate_send(dest, &MqttC$packet[sim_node()]);
}
...
#line 389
          sim_log_debug(161U, "connection", "Connect msg received from Node %u.\n", __nesc_ntoh_uint16(msg->sender.nxdata));

          for (i = 0; i < MqttC$cd_index[sim_node()]; i++) {
              if (MqttC$connection_status_devices[sim_node()][MqttC$cd_index[sim_node()]] == __nesc_ntoh_uint16(msg->sender.nxdata)) {
          ...
          if (!already_connection_status) {
              ...
              MqttC$connection_status_devices[sim_node()][MqttC$cd_index[sim_node()]] = __nesc_ntoh_uint16(msg->sender.nxdata);
              MqttC$cd_index[sim_node()]++;
            }

          MqttC$send_connect_msg_ack(__nesc_ntoh_uint16(msg->sender.nxdata));
        }
      else {
        if (__nesc_ntoh_uint8(msg->type.nxdata) == CONNACK && TOS_NODE_ID != 1 && MqttC$TimerConn$isRunning()) {
            MqttC$TimerConn$stop();
            sim_log_debug(162U, "connection", "ConnAck message in node %d received in time.\n", TOS_NODE_ID);
            MqttC$connection_status[sim_node()] = TRUE;

            MqttC$TimerNextSub$startOneShot(5000);
          }
        else {
          if (__nesc_ntoh_uint8(msg->type.nxdata) == SUB && TOS_NODE_ID == 1) {
              ...
              sim_log_debug(163U, "subscription", "Subsription message received from Node %u.\n", __nesc_ntoh_uint16(msg->sender.nxdata));

              for (i = 0; i < MqttC$cd_index[sim_node()]; i++) {
                  if (MqttC$connection_status_devices[sim_node()][i] == __nesc_ntoh_uint16(msg->sender.nxdata)) {
              ...
              if (node_connection_status) {
                  if (!MqttC$subscription_matrix[sim_node()][__nesc_ntoh_uint8(msg->topic.nxdata) - 1][__nesc_ntoh_uint16(msg->sender.nxdata) - 2]) {
                      sim_log_debug(164U, "subscription", "Node %u subscribed to topic %s(%u).\n\n", __nesc_ntoh_uint16(msg->sender.nxdata), MqttC$topic_name[sim_node()][__nesc_ntoh_uint8(msg->topic.nxdata) - 1], __nesc_ntoh_uint8(msg->topic.nxdata));
                      MqttC$subscription_matrix[sim_node()][__nesc_ntoh_uint8(msg->topic.nxdata) - 1][__nesc_ntoh_uint16(msg->sender.nxdata) - 2] = TRUE;
                    }
                  MqttC$sub_msg_ack(__nesc_ntoh_uint16(msg->sender.nxdata), __nesc_ntoh_uint8(msg->topic.nxdata));
                }
            }
          else {
            if (__nesc_ntoh_uint8(msg->type.nxdata) == SUBACK && TOS_NODE_ID != 1 && MqttC$TimerSub$isRunning()) {
                if (MqttC$subscribed_topic[sim_node()][MqttC$subscribed_topic_index[sim_node()]] == __nesc_ntoh_uint8(msg->topic.nxdata)) {
                    MqttC$TimerSub$stop();
                    sim_log_debug(165U, "connection", "SubAck for topic %s(%u) received in time.\n", MqttC$topic_name[sim_node()][__nesc_ntoh_uint8(msg->topic.nxdata) - 1], __nesc_ntoh_uint8(msg->topic.nxdata));
                    MqttC$current_sub_status[sim_node()] = TRUE;
                    MqttC$subscribed_topic_index[sim_node()]++;
                    MqttC$TimerNextSub$startOneShot(5000);
                  }
              }
            else {
              if (__nesc_ntoh_uint8(msg->type.nxdata) == PUB && TOS_NODE_ID == 1) {
                  ...
                  sim_log_debug(166U, "publish", " Publish message %u received with topic %s(%u) from Node %u.\n", __nesc_ntoh_uint16(msg->value.nxdata), MqttC$topic_name[sim_node()][__nesc_ntoh_uint8(msg->topic.nxdata) - 1], __nesc_ntoh_uint8(msg->topic.nxdata), __nesc_ntoh_uint16(msg->sender.nxdata));
                  ...
                  if (node_connection_status) {
                      MqttC$message_queue[sim_node()][__nesc_ntoh_uint8(msg->topic.nxdata) - 1] = __nesc_ntoh_uint16(msg->value.nxdata);
                      MqttC$sent_index[sim_node()][__nesc_ntoh_uint8(msg->topic.nxdata) - 1] = 0;

                      for (i = 0; i < 3; i++) {
                          if (MqttC$message_list[sim_node()][i] == __nesc_ntoh_uint8(msg->topic.nxdata)) {
#line 465
                            break;
                            }
#line 466
                          if (MqttC$message_list[sim_node()][i] == 0) {
                              MqttC$message_list[sim_node()][i] = __nesc_ntoh_uint8(msg->topic.nxdata);
                            }
                        }
                      sim_log_debug(167U, "publish", " Message %u is queued to be published.\n", __nesc_ntoh_uint16(msg->value.nxdata));
                ...
                if (__nesc_ntoh_uint8(msg->type.nxdata) == PUB && TOS_NODE_ID != 1) {
                    sim_log_debug(168U, "publish", "  Message %u received with topic %s(%u) from the broker.\n", __nesc_ntoh_uint16(msg->value.nxdata), MqttC$topic_name[sim_node()][__nesc_ntoh_uint8(msg->topic.nxdata) - 1], __nesc_ntoh_uint8(msg->topic.nxdata));
                  }
                }
              }
            }
...
# 78 "/opt/tinyos-main-2.1.2/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC$Receive$receive(am_id_t arg_0xb75eeac8, message_t * msg, void * payload, uint8_t len){
#line 78
  nx_struct message_t *__nesc_result;
#line 78

#line 78
  switch (arg_0xb75eeac8) {
#line 78
    case 10:
#line 78
...
#line 78
    default:
#line 78
      __nesc_result = TossimActiveMessageC$Receive$default$receive(arg_0xb75eeac8, msg, payload, len);
#line 78
      break;
#line 78
    }
...
#line 78
# 92 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static bool MqttC$Timer0$isRunning(void ){
#line 92
  unsigned char __nesc_result;
#line 92
...
# 78 "/opt/tinyos-main-2.1.2/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC$Snoop$receive(am_id_t arg_0xb75ec178, message_t * msg, void * payload, uint8_t len){
#line 78
  nx_struct message_t *__nesc_result;
#line 78

#line 78
    __nesc_result = TossimActiveMessageC$Snoop$default$receive(arg_0xb75ec178, msg, payload, len);
#line 78
...
# 110 "/opt/tinyos-main-2.1.2/tos/interfaces/AMSend.nc"
inline static void TossimActiveMessageC$AMSend$sendDone(am_id_t arg_0xb75ee108, message_t * msg, error_t error){
#line 110
  /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$AMSend$sendDone(arg_0xb75ee108, msg, error);
#line 110
}
#line 110
...
# 100 "/opt/tinyos-main-2.1.2/tos/interfaces/Send.nc"
inline static void /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$sendDone(uint8_t arg_0xb74416a8, message_t * msg, error_t error){
#line 100
  switch (arg_0xb74416a8) {
#line 100
    case 0U:
#line 100
...
#line 100
    default:
#line 100
      /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$default$sendDone(arg_0xb74416a8, msg, error);
#line 100
      break;
#line 100
    }
...
#line 171
{
  ...
  if (!MqttC$connection_status[sim_node()]) {
      sim_log_error(149U, "connection", "ACK of connect message not recieved in node %d.\n", TOS_NODE_ID);
      MqttC$send_connect_msg();
    }
}

# 80 "/opt/tinyos-main-2.1.2/tos/interfaces/AMSend.nc"
inline static error_t /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$AMSend$send(am_id_t arg_0xb74400a0, am_addr_t addr, message_t * msg, uint8_t len){
#line 80
  unsigned char __nesc_result;
#line 80

#line 80
  __nesc_result = TossimActiveMessageC$AMSend$send(arg_0xb74400a0, addr, msg, len);
#line 80
...
#line 93
{
  if (MqttC$locked[sim_node()]) {
      return FALSE;
    }
  else {
      if (MqttC$AMSend$send(address, packet, sizeof(mqtt_packet_t )) == SUCCESS) {
...
#line 166
{
  MqttC$actual_send(MqttC$queue_addr[sim_node()], &MqttC$queued_packet[sim_node()]);
}
...
#line 179
{
  ...
  if (!MqttC$current_sub_status[sim_node()]) {
      sim_log_error(150U, "subscription", "ACK of sub message for topic %s(%u) was not recieved in node %d.\n", MqttC$topic_name[sim_node()][MqttC$subscribed_topic[sim_node()][MqttC$subscribed_topic_index[sim_node()]] - 1], MqttC$subscribed_topic[sim_node()][MqttC$subscribed_topic_index[sim_node()]], TOS_NODE_ID);

      MqttC$sub_msg_send(MqttC$subscribed_topic[sim_node()][MqttC$subscribed_topic_index[sim_node()]]);
    }
}
...
#line 191
  if (MqttC$subscribed_topic_index[sim_node()] < 3) {
      if (MqttC$subscribed_topic[sim_node()][MqttC$subscribed_topic_index[sim_node()]] != 0) {
          sim_log_debug(151U, "subscription", "going to next subscription in node %d.\n", TOS_NODE_ID);
          MqttC$current_sub_status[sim_node()] = FALSE;
          MqttC$sub_msg_send(MqttC$subscribed_topic[sim_node()][MqttC$subscribed_topic_index[sim_node()]]);
        }
      else 
        {
...
#line 200
  ...
  if (finished == TRUE && MqttC$publish_topic[sim_node()] != 0) {
      MqttC$TimerNextPub$startOneShot(5000);
    }
}
...
# 52 "/opt/tinyos-main-2.1.2/tos/interfaces/Random.nc"
inline static uint16_t MqttC$Random$rand16(void ){
#line 52
  unsigned short __nesc_result;
#line 52
...
#line 292
{
  mqtt_packet_t *pub_msg = (mqtt_packet_t *)MqttC$Packet$getPayload(&MqttC$packet[sim_node()], sizeof(mqtt_packet_t ));
...
#line 294
  ...
  __nesc_hton_uint8(pub_msg->topic.nxdata, MqttC$publish_topic[sim_node()]);

  __nesc_hton_uint16(pub_msg->value.nxdata, MqttC$Random$rand16() % 100);

  MqttC$TimerNextPub$startOneShot(5000);

  sim_log_debug(156U, "publish", " Publish message with value of %u and topic %s(%u) sent to the broker.\n", __nesc_ntoh_uint16(pub_msg->value.nxdata), MqttC$topic_name[sim_node()][MqttC$publish_topic[sim_node()] - 1], MqttC$publish_topic[sim_node()]);
  MqttC$generate_send(1, &MqttC$packet[sim_node()]);
}
...
# 83 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static void /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$fired(uint8_t arg_0xb72c13a8){
#line 83
  switch (arg_0xb72c13a8) {
#line 83
    case 0U:
#line 83
      MqttC$TimerConn$fired();
#line 83
      break;
#line 83
    case 1U:
...
#line 83
    case 2U:
#line 83
      MqttC$TimerSub$fired();
#line 83
      break;
#line 83
    case 3U:
#line 83
      MqttC$TimerNextSub$fired();
#line 83
      break;
#line 83
    case 4U:
#line 83
      MqttC$TimerNextPub$fired();
#line 83
      break;
#line 83
    default:
#line 83
      /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$default$fired(arg_0xb72c13a8);
#line 83
      break;
#line 83
    }
...
#line 83
#line 73
inline static void MqttC$TimerSub$startOneShot(uint32_t dt){
#line 73
...
# 73 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static void MqttC$TimerConn$startOneShot(uint32_t dt){
#line 73
...
#line 110
{
  ...
  if (TOS_NODE_ID == 2) {
      MqttC$subscribed_topic[sim_node()][1] = MqttC$HUMIDITY[sim_node()];

      MqttC$publish_topic[sim_node()] = MqttC$TEMPRATURE[sim_node()];
    }

  if (TOS_NODE_ID == 3) {
      MqttC$subscribed_topic[sim_node()][0] = MqttC$TEMPRATURE[sim_node()];
    }
  ...
  if (TOS_NODE_ID == 4) {
      ...
      MqttC$publish_topic[sim_node()] = MqttC$LUMINOSITY[sim_node()];
    }

  if (TOS_NODE_ID == 5) {
      MqttC$subscribed_topic[sim_node()][0] = MqttC$HUMIDITY[sim_node()];
      MqttC$subscribed_topic[sim_node()][1] = MqttC$LUMINOSITY[sim_node()];
    }

  if (TOS_NODE_ID == 6) {
      MqttC$publish_topic[sim_node()] = MqttC$HUMIDITY[sim_node()];
    }

  if (TOS_NODE_ID == 7) {
      MqttC$subscribed_topic[sim_node()][0] = MqttC$LUMINOSITY[sim_node()];
      MqttC$subscribed_topic[sim_node()][0] = MqttC$TEMPRATURE[sim_node()];
...
#line 82
{
  if (MqttC$Timer0$isRunning()) {
      return FALSE;
    }
  else {
      MqttC$Timer0$startOneShot(MqttC$time_delays[sim_node()][TOS_NODE_ID - 1]);
      MqttC$queued_packet[sim_node()] = *packet;
      MqttC$queue_addr[sim_node()] = address;
    }
  return TRUE;
}
...
#line 310
{
  uint8_t topic = MqttC$message_list[sim_node()][0];
  int i;
...
#line 316
  ...
  if (MqttC$message_list[sim_node()][0] == 0 && MqttC$message_list[sim_node()][1] == 0 && MqttC$message_list[sim_node()][2] == 0) {
#line 320
    return;
    }
#line 321
  if (MqttC$sent_index[sim_node()][0] < 0 && MqttC$sent_index[sim_node()][1] < 0 && MqttC$sent_index[sim_node()][2] < 0) {
#line 321
    return;
    }
  if (topic == 0 || MqttC$sent_index[sim_node()][topic - 1] < 0) {
      MqttC$message_list[sim_node()][0] = MqttC$message_list[sim_node()][1];
      MqttC$message_list[sim_node()][1] = MqttC$message_list[sim_node()][2];
      MqttC$message_list[sim_node()][2] = 0;
      return MqttC$publish();
    }

  for (i = MqttC$sent_index[sim_node()][topic - 1]; i < 8; i++) {

      if (MqttC$subscription_matrix[sim_node()][topic - 1][i]) {
          destination = i + 2;
          MqttC$sent_index[sim_node()][topic - 1] = i + 1;
          finished = FALSE;
          break;
        }
    }
  ...
  if (finished) {
      MqttC$sent_index[sim_node()][topic - 1] = -1;
      MqttC$message_list[sim_node()][0] = MqttC$message_list[sim_node()][1];
      MqttC$message_list[sim_node()][1] = MqttC$message_list[sim_node()][2];
      MqttC$message_list[sim_node()][2] = 0;
      return MqttC$publish();
  ...
  else 
    {
      ...
      __nesc_hton_uint16(pub_msg->value.nxdata, MqttC$message_queue[sim_node()][topic - 1]);

      sim_log_debug(157U, "publish", " Publish message sent to the node %u.\n", destination);
      MqttC$generate_send(destination, &MqttC$packet[sim_node()]);
    }
}
...
#line 258
{
  mqtt_packet_t *sub_msg = (mqtt_packet_t *)MqttC$Packet$getPayload(&MqttC$packet[sim_node()], sizeof(mqtt_packet_t ));
...
#line 260
  ...
  MqttC$TimerSub$startOneShot(5000);
  sim_log_debug(154U, "subscription", "Subsription message with topic of %s(%u) sent to the broker.\n", MqttC$topic_name[sim_node()][topic - 1], topic);
  MqttC$generate_send(1, &MqttC$packet[sim_node()]);
}
...
#line 488
{
  if (&MqttC$queued_packet[sim_node()] == bufPtr) {
...
#line 494
          for (i = 0; i < 3; i++) {
              if (MqttC$sent_index[sim_node()][i] != -1) {
...
#line 220
{
  mqtt_packet_t *connect_msg = (mqtt_packet_t *)MqttC$Packet$getPayload(&MqttC$packet[sim_node()], sizeof(mqtt_packet_t ));
...
#line 222
  ...
  MqttC$TimerConn$startOneShot(5000);
  sim_log_debug(152U, "connection", "Conn message sent to the broker.\n");
  MqttC$generate_send(1, &MqttC$packet[sim_node()]);
}
                               ...
                               uintptr_t* addr, size_t* size)
{
  ...
  if (!strcmp(varname, "MqttC$queued_packet"))
  {
    *addr = (uintptr_t)&MqttC$queued_packet[__nesc_mote];
    *size = sizeof(MqttC$queued_packet[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$queue_addr"))
  {
    *addr = (uintptr_t)&MqttC$queue_addr[__nesc_mote];
    *size = sizeof(MqttC$queue_addr[__nesc_mote]);
    return 0;
  }
  ...
  if (!strcmp(varname, "MqttC$connection_status"))
  {
    *addr = (uintptr_t)&MqttC$connection_status[__nesc_mote];
    *size = sizeof(MqttC$connection_status[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$current_sub_status"))
  {
    *addr = (uintptr_t)&MqttC$current_sub_status[__nesc_mote];
    *size = sizeof(MqttC$current_sub_status[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$subscribed_topic_index"))
  {
    *addr = (uintptr_t)&MqttC$subscribed_topic_index[__nesc_mote];
    *size = sizeof(MqttC$subscribed_topic_index[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$connection_status_devices"))
  {
    *addr = (uintptr_t)&MqttC$connection_status_devices[__nesc_mote];
    *size = sizeof(MqttC$connection_status_devices[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$cd_index"))
  {
    *addr = (uintptr_t)&MqttC$cd_index[__nesc_mote];
    *size = sizeof(MqttC$cd_index[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$TEMPRATURE"))
  {
    *addr = (uintptr_t)&*(unsigned char *)&MqttC$TEMPRATURE[__nesc_mote];
    *size = sizeof(*(unsigned char *)&MqttC$TEMPRATURE[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$HUMIDITY"))
  {
    *addr = (uintptr_t)&*(unsigned char *)&MqttC$HUMIDITY[__nesc_mote];
    *size = sizeof(*(unsigned char *)&MqttC$HUMIDITY[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$LUMINOSITY"))
  {
    *addr = (uintptr_t)&*(unsigned char *)&MqttC$LUMINOSITY[__nesc_mote];
    *size = sizeof(*(unsigned char *)&MqttC$LUMINOSITY[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$topic_name"))
  {
    *addr = (uintptr_t)&MqttC$topic_name[__nesc_mote];
    *size = sizeof(MqttC$topic_name[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$publish_topic"))
  {
    *addr = (uintptr_t)&MqttC$publish_topic[__nesc_mote];
    *size = sizeof(MqttC$publish_topic[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$sent_index"))
  {
    *addr = (uintptr_t)&MqttC$sent_index[__nesc_mote];
    *size = sizeof(MqttC$sent_index[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$message_queue"))
  {
    *addr = (uintptr_t)&MqttC$message_queue[__nesc_mote];
    *size = sizeof(MqttC$message_queue[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$message_list"))
  {
    *addr = (uintptr_t)&MqttC$message_list[__nesc_mote];
    *size = sizeof(MqttC$message_list[__nesc_mote]);
    return 0;
  }
  ...
  if (!strcmp(varname, "MqttC$subscribed_topic"))
  {
    *addr = (uintptr_t)&MqttC$subscribed_topic[__nesc_mote];
    *size = sizeof(MqttC$subscribed_topic[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "MqttC$subscription_matrix"))
  {
    *addr = (uintptr_t)&MqttC$subscription_matrix[__nesc_mote];
    *size = sizeof(MqttC$subscription_matrix[__nesc_mote]);
    return 0;
  }
...
static void __nesc_nido_initialise(int __nesc_mote)
{
  ...
  memset((void *)&MqttC$queued_packet[__nesc_mote], 0, sizeof MqttC$queued_packet[__nesc_mote]);
  memset((void *)&MqttC$queue_addr[__nesc_mote], 0, sizeof MqttC$queue_addr[__nesc_mote]);
  memcpy((void *)&MqttC$time_delays[__nesc_mote], (void *)&(unsigned short [9]){ 61, 173, 267, 371, 479, 583, 689, 800, 913 }, sizeof MqttC$time_delays[__nesc_mote]);
  MqttC$connection_status[__nesc_mote] = FALSE;
  MqttC$current_sub_status[__nesc_mote] = FALSE;
  MqttC$subscribed_topic_index[__nesc_mote] = 0;
  memset((void *)&MqttC$connection_status_devices[__nesc_mote], 0, sizeof MqttC$connection_status_devices[__nesc_mote]);
  MqttC$cd_index[__nesc_mote] = 0;
  *(unsigned char *)&MqttC$TEMPRATURE[__nesc_mote] = 1;
  *(unsigned char *)&MqttC$HUMIDITY[__nesc_mote] = 2;
  *(unsigned char *)&MqttC$LUMINOSITY[__nesc_mote] = 3;
  memcpy((void *)&MqttC$topic_name[__nesc_mote], (void *)&(char const *[3]){ "temprature", "humidity", "luminosity" }, sizeof MqttC$topic_name[__nesc_mote]);
  MqttC$publish_topic[__nesc_mote] = 0;
  memcpy((void *)&MqttC$sent_index[__nesc_mote], (void *)&(int [3]){ -1, -1, -1 }, sizeof MqttC$sent_index[__nesc_mote]);
  memcpy((void *)&MqttC$message_queue[__nesc_mote], (void *)&(unsigned short [3]){ 0, 0, 0 }, sizeof MqttC$message_queue[__nesc_mote]);
  memcpy((void *)&MqttC$message_list[__nesc_mote], (void *)&(unsigned char [3]){ 0, 0, 0 }, sizeof MqttC$message_list[__nesc_mote]);
  memset((void *)&MqttC$locked[__nesc_mote], 0, sizeof MqttC$locked[__nesc_mote]);
  memcpy((void *)&MqttC$subscribed_topic[__nesc_mote], (void *)&(unsigned char [3]){ 0, 0, 0 }, sizeof MqttC$subscribed_topic[__nesc_mote]);
  memcpy((void *)&MqttC$subscription_matrix[__nesc_mote], (void *)&(unsigned char [3][8]){ { FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE }, { FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE }, { FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE } }, sizeof MqttC$subscription_matrix[__nesc_mote]);
