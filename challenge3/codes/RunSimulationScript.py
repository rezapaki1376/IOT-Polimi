...
# adding a new channel for we are receiving a message.
#print "Activate debug message on channel message_receiving"
#t.addChannel("message_receiving",out);

time = 0*t.ticksPerSecond(); #instant at which each node should be turned on
# initial all the nodes at the time 0. the time is same just nodes changed.
for i in range(1,8):
    print ("Creating node",i,"...");
    node1 =t.getNode(i);
    node1.bootAtTime(time);
    print ">>>Will boot at time", time/t.ticksPerSecond(), "[sec]";
# end of for loop and nodes initialization
